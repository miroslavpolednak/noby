        ...
        : ISbWebApiClient
{
    ...
    {
        var requestUrl = _httpClient.BaseAddress + $"/api/refixationservices/getinterestrate?uver_id={caseId}&date={interestRateValidTo:yyyy-MM-dd}&fixation_time";

        if (fixedRatePeriod.HasValue)
        {
            requestUrl = $"{requestUrl}={fixedRatePeriod}";
        }
    ...
    public async Task<IList<IReadOnlyDictionary<string, string>>> FindTasksByContractNumber(Dto.FindTasks.FindByContractNumberRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            Header = RequestHelper.MapEasHeader(await getLogin(cancellationToken: cancellationToken)),
    ...
    public async Task CancelTask(int taskIdSB, CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        {
            Header = RequestHelper.MapEasHeader(await getLogin("DMS_ADMIN", cancellationToken)),
    ...
    private async Task<string> getLogin(string defaultLogin = "anonymous", CancellationToken cancellationToken = default)
    {
        ...
        if (userId.HasValue)
        {
            ...
            if (string.IsNullOrEmpty(userInstance.UserInfo.Cpm) || string.IsNullOrEmpty(userInstance.UserInfo.Icp))
            {
                var s = userInstance.UserIdentifiers
                                    ...
                                    .FirstOrDefault()?.Identity ?? defaultLogin;
                var idx = s.IndexOf('\\');
