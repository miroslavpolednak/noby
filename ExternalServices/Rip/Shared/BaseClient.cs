    ...
    where TClient : class
{
    ...
    protected async Task<IServiceCallResult> callMethod(Func<Task<IServiceCallResult>> fce)
    {
        ...
        catch (InvalidOperationException e)
        {
            ...
            return new ErrorServiceCallResult(9500, $"Rip Endpoint '{_httpClient.BaseAddress}' unavailable");
        }
        catch (EndpointNotFoundException)
        {
            _logger.LogError("Rip Endpoint '{uri}' not found", _httpClient.BaseAddress);
            return new ErrorServiceCallResult(9501, $"Rip Endpoint '{_httpClient.BaseAddress}' not found");
        }
        catch (Exception e)
        {
            _logger.LogError(e, e.Message);
