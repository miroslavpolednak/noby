...
public static class StartupExtensions
{
    public static IServiceCollection AddExternalServiceEas(this IServiceCollection services, EasConfiguration? easConfiguration)
    {
        ...
        if (easConfiguration.ImplementationType == CIS.Core.ServiceImplementationTypes.Unknown)
        ...
        {
            case Versions.R21:
                if (easConfiguration.ImplementationType == CIS.Core.ServiceImplementationTypes.Mock)
        ...
        services.AddSingleton(provider =>
        {
            ...
            if (easConfiguration.UseServiceDiscovery)
            {
                string? url = provider.GetRequiredService<IDiscoveryServiceAbstraction>()
                    .GetService(new("ES:EAS"), CIS.InternalServices.ServiceDiscovery.Contracts.ServiceTypes.Proprietary)
                    .GetAwaiter()
                    .GetResult()?
                    .ServiceUrl;
