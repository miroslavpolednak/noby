...
internal class LoanApplication3601TemplateData : LoanApplicationBaseTemplateData
{
    ...
    protected override HouseholdInfo CurrentHousehold => HouseholdMain ?? throw CreateNullHouseholdException(nameof(HouseholdMain));

    public LoanApplication3601TemplateData(CustomerWithChangesService customerWithChangesService, ICustomerOnSAServiceClient customerOnSAService) : base(customerWithChangesService)
    {
    ...
    public decimal? LoanPurposes210 => Offer.MortgageOffer.SimulationInputs.LoanPurposes.Where(x => x.LoanPurposeId == 210).Select(x => (decimal?)x.Sum).FirstOrDefault();
    ...
    private string GetDrawingType() =>
        _codebookManager.DrawingTypes.Where(d => d.Id == Offer.MortgageOffer.SimulationInputs.DrawingTypeId)
                        .Select(d => d.Name)
                        .DefaultIfEmpty(string.Empty)
                        .First();

    private int? GetDrawingDuration() =>
        _codebookManager.DrawingDurations.Where(d => d.Id == Offer.MortgageOffer.SimulationInputs.DrawingDurationId)
