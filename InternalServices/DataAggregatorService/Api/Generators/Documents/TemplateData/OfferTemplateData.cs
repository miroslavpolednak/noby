...
internal class OfferTemplateData : AggregatedData
{
    ...
    public string UserName => User.Info.DisplayName;

    public string UserContacts
    {
        get
        {
            var phone = string.IsNullOrWhiteSpace(User.Info.PhoneNumber) ? null : $"telefon: {User.Info.PhoneNumber}";
            var email = string.IsNullOrWhiteSpace(User.Info.Email) ? null : $"e-mail: {User.Info.Email}";
