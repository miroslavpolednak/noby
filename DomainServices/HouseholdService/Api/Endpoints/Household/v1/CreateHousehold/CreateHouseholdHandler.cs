        ...
        : IRequestHandler<CreateHouseholdRequest, CreateHouseholdResponse>
{
    public async Task<CreateHouseholdResponse> Handle(CreateHouseholdRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            await _salesArrangementService.UpdateSalesArrangementState(saInstance.SalesArrangementId, EnumSalesArrangementStates.InSigning, cancellationToken);
        }

        await _responseCache.InvalidateEntry(nameof(GetHouseholdList), request.SalesArrangementId);
