...
internal sealed class CustomersChildMapper
{
    public async Task<List<_C4M.LoanApplicationCounterParty>> MapCustomers(List<_V2.CreditWorthinessCustomer> customers, int? mandantId, CancellationToken cancellation)
    {
        ...
        return customers.Select(t =>
        {
            ...
            // marital status
            _C4M.MartialStatusType maritalStatus = FastEnum.TryParse(maritalStatuses.FirstOrDefault(m => m.Id == t.MaritalStateId)?.RdmCode, out _C4M.MartialStatusType ms) ? ms : _C4M.MartialStatusType.M;

            // Id, IsPartner
            return new _C4M.LoanApplicationCounterParty
            {
                Id = string.IsNullOrEmpty(t.PrimaryCustomerId) ? null : _C4M.ResourceIdentifier.CreateResourceCounterParty(t.PrimaryCustomerId, !mandantId.HasValue || (SharedTypes.Enums.Mandants)mandantId == SharedTypes.Enums.Mandants.Kb ? "KBCZ" : "MPSS").ToC4M(),
