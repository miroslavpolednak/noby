...
internal sealed class DtiRequestMapper
{
    ...
    private static List<_C4M.LoanApplicationCounterparty> mapCustomers(List<_V2.CreditWorthinessCustomer> customers, int? mandantId)
        ...
        {
            CustomerId = string.IsNullOrEmpty(customer.PrimaryCustomerId) ? null : _C4M.ResourceIdentifier.CreateCustomerId(customer.PrimaryCustomerId, !mandantId.HasValue || (SharedTypes.Enums.Mandants)mandantId == SharedTypes.Enums.Mandants.Mp ? "KBCZ" : "MPSS").ToC4M(),
