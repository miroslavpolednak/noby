...
internal class CreateObligationMediatrRequestValidator
    : AbstractValidator<Dto.CreateObligationMediatrRequest>
{
    public CreateObligationMediatrRequestValidator(CodebookService.Abstraction.ICodebookServiceAbstraction codebookService)
    {
        ...
        RuleFor(t => t.Request.CreditCardLimit)
            .Must((r, t) => t.GetValueOrDefault() == 0 || (r.Request.ObligationTypeId.GetValueOrDefault() != 1 && r.Request.ObligationTypeId.GetValueOrDefault() != 2))
            .WithMessage("CreditCardLimit not allowed for current ObligationTypeId").WithErrorCode("0");

        RuleFor(t => t.Request.LoanPrincipalAmount)
            .Must((r, t) => t.GetValueOrDefault() == 0 || (r.Request.ObligationTypeId.GetValueOrDefault() != 3 && r.Request.ObligationTypeId.GetValueOrDefault() != 4))
            .WithMessage("LoanPrincipalAmount not allowed for current ObligationTypeId").WithErrorCode("0");

        RuleFor(t => t.Request.InstallmentAmount)
            .Must((r, t) => t.GetValueOrDefault() == 0 || (r.Request.ObligationTypeId.GetValueOrDefault() != 3 && r.Request.ObligationTypeId.GetValueOrDefault() != 4))
            .WithMessage("InstallmentAmount not allowed for current ObligationTypeId").WithErrorCode("0");
        ...
        RuleFor(t => t.Request.Creditor)
            .ChildRules(v =>
            {
                v.RuleFor(t => t.CreditorId)
                    ...
                    .WithMessage("Creditor.CreditorId and Creditor.Name can't be set in the same time").WithErrorCode("0");
            });
