...
internal class ExceptionInterceptor : Interceptor
{
    ...
    private async Task<TResponse> catcher<TResponse>(AsyncUnaryCall<TResponse> responseAsync, string methodFullName)
    {
        ...
        {
            _logger.LogError(ex, "UserService unavailable");
            throw new ServiceUnavailableException("UserService", methodFullName, ex.Message);
        }
        catch (RpcException ex) when (ex.StatusCode == StatusCode.NotFound)
        {
            throw new CisNotFoundException(1, ex.GetErrorMessageFromRpcException());
        }
