...
namespace DomainServices.CustomerService.Api.Handlers
{
    internal class GetCustomerDetailHandler : IRequestHandler<GetCustomerDetailMediatrRequest, CustomerDetailResponse>
    {
        ...
        public GetCustomerDetailHandler(ILogger<GetCustomerDetailHandler> logger, ICustomerManagementClient customerManagement, ICodebookServiceAbstraction codebooks)
        {
            _logger = logger;
        ...
        public async Task<CustomerDetailResponse> Handle(GetCustomerDetailMediatrRequest request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Get detail instance ID #{id}", request.Request);

            var customer = await _customerManagement.GetDetail(request.Request.Identity.IdentityId, Activity.Current?.TraceId.ToHexString() ?? "", cancellationToken);
