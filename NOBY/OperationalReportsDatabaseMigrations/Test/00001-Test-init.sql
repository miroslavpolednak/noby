...
CREATE SYNONYM [dbo].[NOBY_Case] FOR [ADPRA173\UAT1].[CaseService].[dbo].[Case]

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NOBY_Offer]') AND type in (N'SN'))
	DROP SYNONYM [dbo].[NOBY_Offer]

CREATE SYNONYM [dbo].[NOBY_Offer] FOR [ADPRA173\UAT1].[OfferService].[dbo].[Offer]
...
CREATE SYNONYM [dbo].[NOBY_SalesArrangement] FOR [ADPRA173\UAT1].[SalesArrangementService].[dbo].[SalesArrangement]